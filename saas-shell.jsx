...
(function () {
  ...
  function AuthScreen({ onAuthed, resumeSession, joinCode: joinCodeFromUrl }) {
    ...
    async function doRegister() {
      if (!selectedPlan) { setErr("กรุณาเลือกแผนการใช้งาน"); setScreen("plan"); return; }
      if (!agree) { setErr("กรุณายอมรับเงื่อนไขการใช้บริการก่อนดำเนินการต่อ"); setScreen("plan"); return; }
      setBusy(true); setErr("");
      try {
        let uid;
        if (resumeSession) {
          ...
          if (!su.session) { setNotice("สมัครสำเร็จ กรุณายืนยันอีเมลก่อนเข้าสู่ระบบ"); setScreen("landing"); return; }
        }
    ...
    async function doJoin() {
      ...
      try {
        ...
        if (resumeSession) {
          ...
          if (e1) throw e1;
          uid = su.user && su.user.id;
          if (!su.session) { setNotice("สมัครสำเร็จ กรุณายืนยันอีเมลก่อนเข้าสู่ระบบ"); setScreen("login"); return; }
        }
    ...
    if (screen === "landing") {
      return h("div", { style: shellWrap },
        h("div", { style: cardStyle },
          ...
          h("div", { style: { fontWeight: 700, fontSize: 17, textAlign: "center", marginBottom: 26, color: "#1a5276" } }, "คลินิกเวชกรรมแพทย์ณัฐดนัย"),
          h(ErrBox, { msg: err }),
          h("button", { style: { ...btnStyle, marginBottom: 12 }, onClick: () => { setErr(""); setScreen("login"); } }, "เข้าสู่ระบบ"),
          h("button", {
    ...
    if (screen === "login") {
      return h("div", { style: shellWrap },
        h("div", { style: cardStyle },
          h("div", { style: { fontSize: 40, textAlign: "center", marginBottom: 4 } }, "🏥"),
          h("div", { style: { fontWeight: 700, fontSize: 17, textAlign: "center", marginBottom: 18, color: "#1a5276" } }, "เข้าสู่ระบบ"),
          notice && h("div", { style: noticeStyle }, notice),
          h(ErrBox, { msg: err }),
          ...
          h("button", { style: { ...btnStyle, opacity: busy ? 0.6 : 1 }, disabled: busy, onClick: doLogin }, busy ? "กำลังดำเนินการ..." : "เข้าสู่ระบบ"),
          h("div", { style: { textAlign: "center", marginTop: 14 } },
            h("span", { style: backLinkStyle, onClick: () => { setErr(""); setScreen("landing"); } }, "← กลับ")
          )
        )
      );
    }
    ...
    if (screen === "plan") {
      return h("div", { style: shellWrap },
        h("div", { style: { ...cardStyle, maxWidth: 460 } },
          h("div", { style: { fontWeight: 700, fontSize: 17, textAlign: "center", marginBottom: 4, color: "#1a5276" } }, "แผนการใช้งานและเงื่อนไขบริการ"),
          h("div", { style: { fontSize: 12.5, textAlign: "center", color: "#7f8c8d", marginBottom: 16 } }, "เลือกแผนที่ต้องการก่อนลงทะเบียนคลินิก"),
          h(ErrBox, { msg: err }),
          h("div", { style: { display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 } },
            plans.length === 0 ? h("div", { style: { fontSize: 13, color: "#999" } }, "กำลังโหลดแผน...") :
            plans.map(p => h("div", {
              ...
              h("div", { style: { fontWeight: 700, fontSize: 14.5 } }, p.name),
              h("div", { style: { fontSize: 13, color: "#666", marginTop: 2 } }, "฿" + p.price_thb + "/เดือน"),
              h("div", { style: { fontSize: 12, color: "#999", marginTop: 2 } }, "staff ≤ " + (p.max_staff ?? "ไม่จำกัด") + " · patients ≤ " + (p.max_patients ?? "ไม่จำกัด"))
            ))
          ),
          h("label", { style: { display: "flex", alignItems: "flex-start", gap: 8, margin: "6px 0 18px", fontSize: 13 } },
            h("input", { type: "checkbox", checked: agree, onChange: e => setAgree(e.target.checked), style: { marginTop: 3 } }),
            "ข้าพเจ้าได้อ่านและยอมรับเงื่อนไขการให้บริการและนโยบายความเป็นส่วนตัวของระบบ"
          ),
          h("button", {
    ...
    if (screen === "register") {
      ...
      return h("div", { style: shellWrap },
        h("div", { style: { ...cardStyle, maxWidth: 460 } },
          h("div", { style: { fontWeight: 700, fontSize: 17, textAlign: "center", marginBottom: 4, color: "#1a5276" } }, "ลงทะเบียนคลินิกใหม่"),
          planName && h("div", { style: { fontSize: 12.5, textAlign: "center", color: "#7f8c8d", marginBottom: 16 } }, "แผนที่เลือก: " + planName.name),
          notice && h("div", { style: noticeStyle }, notice),
          h(ErrBox, { msg: err }),
          h("input", { style: inputStyle, type: "email", placeholder: "อีเมล", value: email, onChange: e => setEmail(e.target.value) }),
          h("input", { style: inputStyle, type: "password", placeholder: "รหัสผ่าน", value: password, onChange: e => setPassword(e.target.value) }),
          h("input", { style: inputStyle, placeholder: "ชื่อ-นามสกุล", value: fullName, onChange: e => setFullName(e.target.value) }),
          h("input", { style: inputStyle, placeholder: "ชื่อคลินิก", value: clinicName, onChange: e => setClinicName(e.target.value) }),
          h("input", { style: inputStyle, placeholder: "เลขที่ใบอนุญาตประกอบวิชาชีพเวชกรรม", value: licenseNumber, onChange: e => setLicenseNumber(e.target.value) }),
          h("input", { style: inputStyle, placeholder: "ที่อยู่คลินิก", value: address, onChange: e => setAddress(e.target.value) }),
          h("input", { style: inputStyle, placeholder: "เบอร์โทรศัพท์", value: phone, onChange: e => setPhone(e.target.value) }),
          h("button", { style: { ...btnStyle, opacity: busy ? 0.6 : 1 }, disabled: busy, onClick: doRegister }, busy ? "กำลังส่งข้อมูล..." : "ลงทะเบียน"),
          h("div", { style: { textAlign: "center", marginTop: 14 } },
            h("span", { style: backLinkStyle, onClick: () => { setErr(""); setScreen("plan"); } }, "← กลับไปเลือกแผน")
          )
        )
      );
    }
    ...
    return h("div", { style: shellWrap },
      h("div", { style: cardStyle },
        h("div", { style: { fontSize: 40, textAlign: "center", marginBottom: 4 } }, "🏥"),
        h("div", { style: { fontWeight: 700, fontSize: 17, textAlign: "center", marginBottom: 18, color: "#1a5276" } }, "เข้าร่วมคลินิก"),
        notice && h("div", { style: noticeStyle }, notice),
        h(ErrBox, { msg: err }),
        h("input", { style: inputStyle, placeholder: "ชื่อ-นามสกุล", value: fullName, onChange: e => setFullName(e.target.value) }),
        !joinCodeFromUrl && h("input", { style: inputStyle, placeholder: "รหัสคลินิก (จากแพทย์)", value: joinCode, onChange: e => setJoinCode(e.target.value) }),
        h("input", { style: inputStyle, type: "email", placeholder: "อีเมล", value: email, onChange: e => setEmail(e.target.value) }),
        h("input", { style: inputStyle, type: "password", placeholder: "รหัสผ่าน", value: password, onChange: e => setPassword(e.target.value) }),
        h("button", { style: { ...btnStyle, opacity: busy ? 0.6 : 1 }, disabled: busy, onClick: doJoin }, busy ? "กำลังดำเนินการ..." : "ลงทะเบียนเข้าร่วม"),
        h("div", { style: { textAlign: "center", marginTop: 14 } },
          h("span", { style: backLinkStyle, onClick: () => { setErr(""); setScreen("login"); } }, "← กลับไปหน้าเข้าสู่ระบบ")
        )
      )
    );
  }
  ...
  function ConfirmScreen({ tokenHash, type, onDone }) {
    ...
    return h("div", { style: shellWrap },
      h("div", { style: cardStyle },
        h("div", { style: { fontSize: 40, textAlign: "center", marginBottom: 4 } }, "📧"),
        h("div", { style: { fontWeight: 700, fontSize: 17, textAlign: "center", marginBottom: 12, color: "#1a5276" } }, "ยืนยันอีเมลของคุณ"),
        h("p", { style: { fontSize: 13.5, color: "#555", textAlign: "center", marginBottom: 16 } }, "กดปุ่มด้านล่างเพื่อยืนยันตัวตนและเข้าสู่ระบบ (ต้องกดด้วยตนเองเพื่อป้องกันลิงก์ถูกใช้โดยระบบสแกนอีเมลอัตโนมัติ)"),
        h(ErrBox, { msg: err }),
        h("button", { style: { ...btnStyle, opacity: busy ? 0.6 : 1 }, disabled: busy, onClick: confirm },
  ...
  function SuperAdminPortal() {
    ...
    async function loadAll() {
      ...
      setClinics(c.data || []); setSubs(s.data || []); setPlans(p.data || []); setOwners(o.data || []);
      setLoading(false);
    }
    ...
    async function approveClinic(clinic) {
      ...
      if (owner) await sb.from("profiles").update({ status: "approved" }).eq("id", owner.id);
      if (!subFor(clinic.id)) {
    ...
    async function rejectClinic(clinic) {
      if (!window.confirm("ยืนยันปฏิเสธคำขอลงทะเบียนคลินิกนี้?")) return;
      await sb.from("clinics").update({ status: "rejected" }).eq("id", clinic.id);
      const owner = ownerFor(clinic.id);
      if (owner) await sb.from("profiles").update({ status: "rejected" }).eq("id", owner.id);
      loadAll();
    }
    ...
    function backToAdmin() {
      if (prevTenant.current) { window.__TENANT__.clinicId = prevTenant.current.clinicId; window.__TENANT__.accessToken = prevTenant.current.accessToken; }
      try { localStorage.removeItem("clinic_session_v1"); } catch (_) {}
      setOpenClinic(null);
    }

    if (openClinic) {
      return h("div", null,
        h("div", { style: { background: COLORS.primaryDark, color: "#fff", padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13.5 } },
          h("div", null, "🛠 กำลังดูในฐานะ Super Admin: ", h("b", null, openClinic.name)),
          h("button", { onClick: backToAdmin, style: { background: "rgba(255,255,255,0.15)", color: "#fff", border: "1px solid rgba(255,255,255,0.5)", borderRadius: 6, padding: "5px 12px", cursor: "pointer", fontSize: 13 } }, "← กลับสู่ Super Admin")
        ),
        h(window.AppRoot)
      );
    }
    ...
    return h("div", { style: { minHeight: "100vh", background: COLORS.bg, fontFamily: "'Sarabun', sans-serif" } },
      ...
      h("div", { style: { display: "flex", gap: 8, padding: "14px 20px" } },
        ["pending", "clinics", "plans"].map(k => h("button", {
        ...
        }, k === "pending" ? `รออนุมัติ${pendingClinics.length ? " (" + pendingClinics.length + ")" : ""}` : k === "clinics" ? "คลินิกทั้งหมด" : "แผนราคา"))
      ),
      loading ? h("div", { style: { padding: 20 } }, "กำลังโหลด...") :

      tab === "pending" ? h("div", { style: { padding: "0 20px 20px", display: "flex", flexDirection: "column", gap: 12 } },
        pendingClinics.length === 0 ? h("div", { style: { color: "#999", fontSize: 13.5 } }, "ไม่มีคำขอลงทะเบียนที่รออนุมัติ") :
        pendingClinics.map(c => {
          const owner = ownerFor(c.id);
          const s = subFor(c.id);
          return h("div", { key: c.id, style: { background: "#fff", borderRadius: 10, padding: 16 } },
            h("div", { style: { fontWeight: 700, fontSize: 15 } }, c.name),
            h("div", { style: { fontSize: 13, color: "#555", marginTop: 6, lineHeight: 1.7 } },
              "ผู้ลงทะเบียน: " + (owner ? owner.full_name : "-") + (owner && owner.email ? " · " + owner.email : ""), h("br"),
              "เลขที่ใบอนุญาต: " + (owner && owner.medical_license_number ? owner.medical_license_number : "-"), h("br"),
              "ที่อยู่: " + (c.address || "-"), h("br"),
              "โทร: " + (c.phone || "-"), h("br"),
              "แผนที่เลือก: " + (s && s.plans ? s.plans.name : "-")
            ),
            h("div", { style: { display: "flex", gap: 8, marginTop: 12 } },
              h("button", { onClick: () => approveClinic(c), style: { flex: 1, fontSize: 13, padding: "8px 10px", borderRadius: 6, border: "none", cursor: "pointer", background: "#1e8449", color: "#fff", fontWeight: 600 } }, "✓ อนุมัติ"),
              h("button", { onClick: () => rejectClinic(c), style: { flex: 1, fontSize: 13, padding: "8px 10px", borderRadius: 6, border: "none", cursor: "pointer", background: "#c0392b", color: "#fff", fontWeight: 600 } }, "✕ ปฏิเสธ")
            )
          );
        })
      ) :
        ...
        h("table", { style: { width: "100%", borderCollapse: "collapse", background: "#fff", borderRadius: 10, overflow: "hidden" } },
          ...
          h("tbody", null, clinics.map(c => {
            ...
            return h("tr", { key: c.id, style: { borderTop: "1px solid #eee" } },
              ...
              h("td", { style: { padding: 10, fontSize: 13.5, color: c.status === "active" ? "#1e8449" : c.status === "pending" ? "#b7791f" : "#c0392b" } }, c.status),
              ...
              h("td", { style: { padding: 10, display: "flex", gap: 6 } },
                c.status !== "pending" && h("button", {
  ...
  function RootRouter() {
    ...
    async function loadAll(session) {
      try {
        ...
        if (profile.status === "rejected") { setState({ phase: "rejected", role: profile.role }); return; }
        if (profile.status === "pending") { setState({ phase: "pending", role: profile.role }); return; }
        ...
        if (clinic && clinic.status === "rejected") { setState({ phase: "rejected", role: profile.role }); return; }
        if (clinic && clinic.status === "pending") { setState({ phase: "pending", role: profile.role }); return; }
    ...
    React.useEffect(() => {
      if (pendingConfirm) {
        ...
        withTimeout(sb.auth.getSession(), 10000, "โหลดเซสชัน")
          .then(({ data }) => { if (data.session) loadAll(data.session); else setState({ phase: "auth" }); })
      ...
      const { data: sub } = sb.auth.onAuthStateChange((_e, session) => {
        ...
        else if (!pendingConfirm) { try { localStorage.removeItem("clinic_session_v1"); } catch (_) {} setState({ phase: "auth" }); }
      });
